...
* The "MIDI" grid I would like to have access to all the lamps, but with only ONE midi-control.
  My suggestion is that when you move the mouse over the grid, you send the value of the "lamp" you are pointing at to the midi control channel. This way we make sure that the control knob and the lamp is synchronized so the lamp wont jump in brightness when you start turning.  It will allso help you finding the right lamp by viewing the leds on the knob.
